...
- **File uploads (presigned-URL flow):** agents upload files via a three-step flow — no base64, no separate auth header. (1) Call `prepare_upload` with `filename`, `contentType`, and `size` to receive a short-lived signed `uploadUrl` and an `uploadHandle`. (2) PUT raw bytes directly to `uploadUrl` with the matching `Content-Type` header — no `Authorization` header is needed (the capability is in the signed URL). (3) Call `finalize_upload` with the `uploadHandle` to get back `{ key }` — a stable blob key for use in `create_note` (photoKeys) or a planter's `photoBlobKey`. The signed URL expires in 5 minutes and is single-use. New uploads land in the `uploads` blob store at `staging/<uploadId>`; legacy uploads (pre-this-change) remain in the `photos` store. The `blob-cleanup` service scans both stores for orphans.
